...
void setup()
{
  ...
  SerialUSB.println("Waiting for GPS time ... ");
  smartDelay(1000);
  delay(2000);
...
void loop()
{
  ...
  if (Calculate_A_Phasor == true)
  {
    ...
    SerialUSB.print("Time for 1 Set of Sample Acquisition is ");
    SerialUSB.print(acquisition_finish_millis - acquisition_start_millis);
    SerialUSB.print(" ms or ");
    SerialUSB.print(acquisition_finish_micros - acquisition_start_micros);
    SerialUSB.println(" us ");

    SerialUSB.print("Time for 1 Set of Phasors Calculation is ");
    SerialUSB.print(calculation_finish_millis - calculation_start_millis);
    SerialUSB.print(" ms or ");
    SerialUSB.print(calculation_finish_micros - calculation_start_micros);
    SerialUSB.println(" us ");

    transmission_start_micros = micros();
    transmission_start_millis = millis();
    transmit_phasors_on_Serial1();
    Transmit_Phasors_on_SerialUSB();
    transmission_finish_micros = micros();
    transmission_finish_millis = millis();

    SerialUSB.print("Time for 1 Set of Phasors Transmission is ");
    SerialUSB.print(transmission_finish_millis - transmission_start_millis);
    SerialUSB.print(" ms or ");
    SerialUSB.print(transmission_finish_micros - transmission_start_micros);
    SerialUSB.println(" us ");
    SerialUSB.println("");
...
//
//}
void Transmit_Phasors_on_SerialUSB() {
  SerialUSB.print("Phasor Magnitude for Phase 1 is: ");
  SerialUSB.println(float(Phasor_Magnitude_1));
  SerialUSB.print("Phasor Magnitude for Phase 2 is: ");
  SerialUSB.println(float(Phasor_Magnitude_2));
  SerialUSB.print("Phasor Magnitude for Phase 3 is: ");
  SerialUSB.println(float(Phasor_Magnitude_3));

  SerialUSB.print("Phase 1 Angle is: ");
  SerialUSB.println(float(Phasor_Angle_Degree_1));
  SerialUSB.print("Phase 2 Angle is: ");
  SerialUSB.println(float(Phasor_Angle_Degree_2));
  SerialUSB.print("Phase 3 Angle is: ");
  SerialUSB.println(float(Phasor_Angle_Degree_3));
  ...
  if (hour() < 10) SerialUSB.print(F("0"));
  SerialUSB.print(hour());
  SerialUSB.print(F(":"));
  if (minute() < 10) SerialUSB.print(F("0"));
  SerialUSB.print(minute());
  SerialUSB.print(F(":"));
  if (second() < 10) SerialUSB.print(F("0"));
  SerialUSB.print(second());
  SerialUSB.print(F(":"));
  SerialUSB.print(calculation_start_millis - pps_time_millis);


  SerialUSB.print(" ");

  if (day() < 10) SerialUSB.print(F("0"));
  SerialUSB.print(day());
  SerialUSB.print(F("/"));
  if (month() < 10) SerialUSB.print(F("0"));
