...
int N = 100;//Change no of samples here

double adc_out[100];
double values[100];

long int delay_between_samples=20000/N;
...
void loop()
{
  ...
  if (Calculate_A_Phasor == true)
  {
    ...
    Zi = 0;
    Zr = 0;
    //Take N samples for 1 cycle Window To calculate phasor
    for (int i = 0; i < N; i++)
    {
      //Read adc
      adc_out[i] = analogRead(A0);
      
      delayMicroseconds(delay_between_samples); //40 uS Delay for 500 samples for 1 cycle
    }
